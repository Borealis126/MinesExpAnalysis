...
class Pulse(object):
    ...
    @property
    def tzForm(self):
        return [self.amp, self.duration, self.phase, self.shapeFunc]

    @property
    def view(self):
        return ["A: "+str(round(self.amp, 2)),
                "t: "+str(round(self.duration/1e-9, 2))+"ns",
                "phi: "+str(round(self.phase, 2)),
                "shape: "+self.shapeFuncName]

class Op(object):  # A list of pulses
    ...
    @property
    def view(self):
        return [["A: "+str(round(pulse.amp, 2)),
                 "t: "+str(round(pulse.duration/1e-9, 2))+"ns",
                 "phi: "+str(round(pulse.phase, 2))]
...
class Exp(object):  # A list of ExpSlices
...
class BatchExp(object): # A list of Exps
...
def durationTrunc(t):
    res = 1/1.2e9*4 #Based on the sampling rate
