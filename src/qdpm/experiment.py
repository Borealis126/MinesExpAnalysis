...
class Experiment(object):
    ...
    def __init__(
        ...
        self.__repeatRange = repeatRange
    ...
    def _setting_assembly(self, scan_init_file, scan_setup_file,
        ...
        if self.__path.joinpath('assets/settings.joblib').exists():
            ...
            self.__scan_value_raw = pd.read_csv(
                ...
                float_precision='round_trip').to_numpy().copy()
                ...
                if ew_element.find('{http://www.ni.com/LVData}Val').text == '5':
                    for u32 in root.iter('{http://www.ni.com/LVData}U32'):
                        if u32.find('{http://www.ni.com/LVData}Name').text == '#rec':
                            ...
                            # in case average number is shorter than
                            # sequence number, self.__scan_value_raw
                            # is stretched to the maximun size
                            self.__scan_value_raw.resize((len(self.__scan_size), max(self.__scan_size)))
    ...
    def _get_data(self):
        minRepeat = 0
        maxRepeat = None
        if self.__repeatRange != 'All':
            minRepeat, maxRepeat = self.__repeatRange
            self.__scan_size[1] = maxRepeat-minRepeat
                ...
                for res in range(self.__number_of_readout):
                    for scan in range(self.__scan_size[-1]):
                        self.__r[res][0][:, :, scan] = pd.read_csv(
                            ...
                            index_col=None,
                            float_precision='round_trip',
                            header=None).T.to_numpy()[:, minRepeat:maxRepeat]
