...
class QdpmFit(object):
    ...
    def make_plots(self):
        ...
        if self.ax is None:
            plt.figure()
            plt.plot(self.xpts, self.ypts, "b.", markersize=10, label="Data")
            xdata = np.linspace(self.xpts.min(), self.xpts.max(), 1000)
            plt.plot(xdata, self.model(xdata), "r-", linewidth=2, label="Fit")
            ...
            self.ax.plot(self.xpts, self.ypts, "b.", markersize=10, label="Data")
            xdata = np.linspace(self.xpts.min(), self.xpts.max(), 1000)
            self.ax.plot(xdata, self.model(xdata), "r-", linewidth=2, label="Fit")
    ...
    def _do_fit(self, bounds=None):
        ...
        popt, pcov = curve_fit(self._model, self.xpts, self.ypts, p0, method='lm')
...
class MultiGaussianFit(QdpmFit):
