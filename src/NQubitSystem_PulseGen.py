...
class Qubit_PG(Qubit):
    def __init__(self, index, otherQubitIndices):
        ...
        self.RofPhiData_RT = list()
        self.RofPhi_RT_interp = lambda x, y: 1 #Placeholder
    ...
    def set_RofPhi_RT_interp(self):
        self.RofPhi_RT_interp =  lambda R, phi: RectBivariateSpline(self.RofPhiData_RT[1:, 0],
                                                                    self.RofPhiData_RT[0, 1:],
                                                                    self.RofPhiData_RT[1:, 1:])(R, phi)[0, 0]
    ...
    def RofPhi_RT_correction(self, R, phi):
        if R > -0.1 and R < 0.1:
            return R
        else:
            return R/self.RofPhi_RT_interp(R, phi)


class NQubitSystem_PG(NQubitSystem):
    def __init__(self, paramsDirectory):
        super(NQubitSystem_PG, self).__init__(paramsFilePath=str(paramsDirectory / 'qSysParams.csv'))
        self.maxAPSAmp = 0
        self.calibrationDirectory = str()
        self.paramsDirectory = paramsDirectory
    ...
    def loadQSysParams_PG(self):
        ...
        for qubitIndex, q in enumerate(self.qubits):
            ...
            # q.RofPhiData_RT = np.loadtxt(self.paramsDirectory / ('RofPhi_Ch'+str(qubitIndex)+'.csv'), delimiter=',')
            # q.set_RofPhi_RT_interp()
